...
def generate_core_ivf(dataset_name: str, normalize=True):
    ...
    if os.path.exists(idx_path):
        print(f'IVF Index already exists for {dataset_name}. Skipping.')
        return
...
def generate_core_ivf2(dataset_name: str, normalize=True):
    ...
    if normalize:
        data = preprocessing.normalize(data, axis=1, norm='l2')
    core_idx = IVF2(DIMENSIONALITIES[dataset_name], 'l2sq', nbuckets, nbuckets_l0)
    training_points = nbuckets * 300  # Our collections do not need that many training points
    if training_points < num_embeddings:
        rng = default_rng()
        training_sample_idxs = rng.choice(num_embeddings, size=training_points, replace=False)
...
if __name__ == "__main__":
    ...
    # generate_core_ivf2('coco-nomic-768-normalized', normalize=True)
    # generate_core_ivf2('simplewiki-openai-3072-normalized', normalize=True)
    # generate_core_ivf2('imagenet-align-640-normalized', normalize=True)
    # generate_core_ivf2('imagenet-clip-512-normalized', normalize=True)
    # generate_core_ivf2('laion-clip-512-normalized', normalize=True)
    # generate_core_ivf2('codesearchnet-jina-768-cosine', normalize=True)
    # generate_core_ivf2('yi-128-ip', normalize=True)
    # generate_core_ivf2('landmark-dino-768-cosine', normalize=True)
    # generate_core_ivf2('landmark-nomic-768-normalized', normalize=True)
    # generate_core_ivf2('arxiv-nomic-768-normalized', normalize=True)
    generate_core_ivf2('ccnews-nomic-768-normalized', normalize=True)
    # generate_core_ivf2('celeba-resnet-2048-cosine', normalize=True)
    # generate_core_ivf2('llama-128-ip', normalize=True)
    # generate_core_ivf2('yandex-200-cosine', normalize=True)
    # generate_core_ivf2('word2vec-300', normalize=True)
    # generate_core_ivf2('sift-128-euclidean', normalize=True)
    # generate_core_ivf2('openai-1536-angular', normalize=True)
    # generate_core_ivf2('msong-420', normalize=True)
    # generate_core_ivf2('instructorxl-arxiv-768', normalize=True)
    # generate_core_ivf2('contriever-768', normalize=True)
    # generate_core_ivf2('gist-960-euclidean', normalize=True)
    # generate_core_ivf2('yahoo-minilm-384-normalized', normalize=True)
    # generate_core_ivf2('gooaq-distilroberta-768-normalized', normalize=True)
    # generate_core_ivf2('agnews-mxbai-1024-euclidean', normalize=True)
    # generate_core_ivf2('glove-200-angular', normalize=True)
